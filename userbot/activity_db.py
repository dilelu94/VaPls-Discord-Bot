...
def get_last_voice_timestamps(guild_id: int) -> dict[int, int]:
    """Return {user_id: last_unix_ts} from voice channel activity, grouped by user."""
    if _conn is None:
        return {}
    res: dict[int, int] = {}
    voice_types = ("voice_session", "voice_join", "voice_vad")
    placeholders = ",".join("?" * len(voice_types))

    cur = _conn.execute(
        f"""SELECT user_id, MAX(created_at) AS last_seen
           FROM activity_log
           WHERE guild_id=? AND activity_type IN ({placeholders})
           GROUP BY user_id""",
        (guild_id, *voice_types),
    )
    for row in cur.fetchall():
