...
@bot.slash_command(
    name="mirarindio", description="Hace que el indio mire el stream del usuario en voz"
)
async def mirarindio(
    ctx,
    usuario: discord.Option(
        discord.Member,
        description="Usuario que está transmitiendo (opcional)",
        required=False,
        default=None,
    ) = None,
):
    """Slash command: ask the userbot to watch a GoLive stream in voice."""
    await safe_defer(ctx)
    _track_command(ctx, "mirarindio")

    if ctx.guild is None:
        await safe_respond(ctx, "❌ Este comando solo se puede usar en un servidor.")
        return

    target_member = usuario or ctx.author
    streamer_name = getattr(target_member, "display_name", str(target_member))
    streamer_id = getattr(target_member, "id", None)

    if not (config.INDIO_RELAY_URL and config.INDIO_RELAY_SECRET):
        await safe_respond(ctx, "❌ El relay del indio no está configurado.")
        return

    url = urljoin(config.INDIO_RELAY_URL, "/stream-watch")
    headers = {"X-API-Secret": config.INDIO_RELAY_SECRET}
    payload = {
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as sess:
            async with sess.post(url, json=payload, headers=headers) as resp:
                if resp.status >= 400:
                    await safe_respond(ctx, f"⚠️ El indio no pudo mirar el stream (HTTP {resp.status}).")
                    return
    except Exception as e:
        log.exception("mirarindio relay failed")
        await safe_respond(ctx, f"⚠️ Error llamando al indio: {e}")
        return

    await safe_respond(ctx, f"👀 El indio se puso a mirar el stream de **{streamer_name}**.")


@bot.slash_command(
    name="dejardemirar", description="Hace que el indio deje de mirar el stream"
)
async def dejardemirar(ctx):
    """Slash command: ask the userbot to stop watching the stream in voice."""
    await safe_defer(ctx)
    _track_command(ctx, "dejardemirar")
    ...
    url = urljoin(config.INDIO_RELAY_URL, "/stream-unwatch")
    headers = {"X-API-Secret": config.INDIO_RELAY_SECRET}
    payload = {"guild_id": int(ctx.guild.id)}
    timeout = aiohttp.ClientTimeout(total=config.INDIO_RELAY_TIMEOUT)
    try:
        async with aiohttp.ClientSession(timeout=timeout) as sess:
            async with sess.post(url, json=payload, headers=headers) as resp:
                if resp.status >= 400:
                    await safe_respond(ctx, f"⚠️ Error al detener espectador (HTTP {resp.status}).")
                    return
    except Exception as e:
        log.exception("dejardemirar relay failed")
        await safe_respond(ctx, f"⚠️ Error llamando al indio: {e}")
        return

    await safe_respond(ctx, "🚪 El indio dejó de mirar el stream.")
...
async def sensibilidad(
    ...
    _PRESET_DESCRIPTIONS = {
        0: "**Preset 0** — desactivar todo el sistema de escucha del indio.",
