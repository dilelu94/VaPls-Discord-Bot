...
Cada usuario tiene "pronombres: él" o "pronombres: ella" en sus rasgos. \
Usalos para la concordancia al hablar de ellos. \
...
async def ask_indio_stream_vision(
    *,
    ...
    """
    if not image_bytes:
        logger.info("[STREAM-VISION] Imagen vacía recibida para stream de %s — omitiendo", streamer_name)
        return None
    ...
    if streamer_id and streamer_id in _USERS:
        ...
        s_name_lower = streamer_name.lower()
        for uid, uinfo in _USERS.items():
            uname = str(uinfo.get("name", "")).lower()
            if uname == s_name_lower or s_name_lower in uname:
                u_data = uinfo
                streamer_id = uid
                break

    if u_data:
        canonical_name = u_data.get("name", streamer_name)
        traits = u_data.get("traits", [])
        anecdotas = u_data.get("anecdotas", [])
        preguntas = u_data.get("preguntas_tipicas", [])
        details = []
        if traits:
            details.append("Rasgos/Personalidad: " + ", ".join(str(t) for t in traits))
        if anecdotas:
            details.append("Anécdotas conocidas: " + "; ".join(str(a) for a in anecdotas))
        if preguntas:
            details.append("Frases/Preguntas típicas: " + "; ".join(str(p) for p in preguntas))

        user_info_str = f" Contexto detallado de tu amigo {canonical_name}: " + " | ".join(details) + "."
        streamer_name = canonical_name
        logger.info(
            "[STREAM-VISION] Asociando memoria del usuario %s (ID: %s) -> %d rasgos, %d anécdotas",
            canonical_name,
            streamer_id,
            len(traits),
            len(anecdotas),
        ...
        logger.info(
            "[STREAM-VISION] No se encontró perfil de usuario para streamer_name='%s' (id=%s)",
            streamer_name,
            streamer_id,
        )

    logger.info(
        "[STREAM-VISION] Enviando captura a Gemini Vision (streamer=%s, id=%s, guild=%s, bytes=%d)...",
        streamer_name,
        streamer_id,
        guild_id,
        len(image_bytes),
    ...
    prompt = (
        f"Estás viendo la transmisión en vivo de GoLive de tu amigo {streamer_name}.{user_info_str}\n"
    ...
    try:
        ...
        if not clean_text or clean_text.upper() == "SKIP" or clean_text.upper().startswith("SKIP"):
            logger.info("[STREAM-VISION] Resultado para stream de %s: SKIP (pantalla sin novedad)", streamer_name)
            return None

        logger.info("[STREAM-VISION] Comentario generado para stream de %s (ID: %s): '%s'", streamer_name, streamer_id, clean_text)
        ...
        if guild_id:
            try:
                ...
                lock = _indio_locks.setdefault(hist_key, asyncio.Lock())
                ts = time.time()
                obs_prompt = _sanitize_for_history(f"[Observación de la transmisión en vivo de GoLive de {streamer_name}]")
                obs_reply = _sanitize_for_history(clean_text)
                user_turn = {"role": "user", "parts": [{"text": obs_prompt[:_STORED_MSG_MAX_CHARS]}], "ts": ts}
                model_turn = {"role": "model", "parts": [{"text": obs_reply[:_STORED_MSG_MAX_CHARS]}], "ts": ts}
                async with lock:
                    history = list(_indio_history.get(hist_key, []))
                ...
                logger.warning("[STREAM-VISION] No se pudo guardar comentario de stream en memoria corta: %s", hist_err)

        return clean_text
    except Exception as e:
        logger.warning("[STREAM-VISION] ask_indio_stream_vision falló para %s: %s", streamer_name, e)
        return None
