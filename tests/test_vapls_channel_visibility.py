...
def test_allowlist_matches_the_two_channels():
    # The invariant is pinned to exactly these two ids; guards against a config
    # drift silently widening where the bot may post publicly.
    assert config.PUBLIC_ALLOWED_CHANNEL_IDS == {REPLY_CHANNEL, PLAY_CHANNEL}


async def test_reply_public_in_reply_channel(ctx_factory, patch_generate, reply_factory):
...
async def test_reply_ephemeral_in_other_channel(ctx_factory, patch_generate, reply_factory):
    patch_generate(reply=reply_factory(text=REPLY))
...
async def test_long_reply_fully_ephemeral_in_other_channel(ctx_factory, patch_generate, reply_factory):
