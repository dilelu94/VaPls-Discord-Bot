...
@pytest.mark.asyncio
async def test_speak_indio_reply_uses_configured_guild_voice_effect(monkeypatch):
    posted_payload = {}
    ...
    monkeypatch.setattr(aiohttp, "ClientSession", MockSession)
    geminiCommand._indio_voice_effects["guild-777"] = "alien"

    member = MagicMock(name="Member")
    member.id = 123
    member.voice = SimpleNamespace(channel=MagicMock(id=456))

    await geminiCommand._speak_indio_reply(None, 777, member, "Probando efecto alien guardado.")
