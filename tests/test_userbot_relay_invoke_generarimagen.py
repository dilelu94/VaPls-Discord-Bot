"""Behavioral tests for the /invoke_generarimagen endpoint on the userbot relay.
...
def _load_invoke_genimagen_handler():
    ...
    handler_block = _extract("_relay_invoke_generarimagen")
    ...
    log_stub = logging.getLogger("test_invoke_generarimagen")
    ...
    return ns["_relay_invoke_generarimagen"], cfg, cli


_handler, _cfg, _client = _load_invoke_genimagen_handler()


def _make_app() -> web.Application:
    app = web.Application()
    app.router.add_post("/invoke_generarimagen", _handler)
    return app
...
async def test_invoke_generarimagen_success():
...
async def test_invoke_generarimagen_returns_504_when_stalled():
...
async def test_invoke_generarimagen_returns_429_on_rate_limit():
