...
async def startApiServer(bot: discord.Bot) -> web.AppRunner:
    ...
    if config.INSTAGRAM_PAGE_TOKEN and config.INSTAGRAM_PAGE_ID:
        ...
        asyncio.create_task(_poll_instagram_media(bot))
        logger.info("Instagram DM and Media polling started")
...
_seen_instagram_media_ids: set[str] = set()


async def _poll_instagram_media(bot: discord.Bot) -> None:
    """Poll the Instagram business account's published media every 60s.

    Discovers new posts (images/videos/reels) made by the Indio's own Instagram
    account. Posts them in the Discord text channel, and streams if they are videos.
    """
    from instagramCommand import start_instagram_reel_stream_logic

    global _seen_instagram_media_ids
    POLL_INTERVAL = 60  # check posts every 60 seconds
    API_BASE = "https://graph.facebook.com/v25.0"
    ...
    if not page_token or not page_id:
        logger.error("[INSTAGRAM-MEDIA] Page token or Page ID missing — media polling disabled")
        return

    # 1. Resolve the connected Instagram Business Account ID
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url) as resp:
                if resp.status == 200:
                    ...
                    if insta_acc_id:
                        logger.info(f"[INSTAGRAM-MEDIA] Resolved Instagram Business Account ID: {insta_acc_id}")
                else:
                    body = await resp.text()
                    logger.warning(f"[INSTAGRAM-MEDIA] Failed to resolve Instagram business account {resp.status}: {body}")
    except Exception as e:
        logger.warning(f"[INSTAGRAM-MEDIA] Error resolving Instagram business account: {e}")

    if not insta_acc_id:
        logger.error("[INSTAGRAM-MEDIA] No Instagram Business Account found — media polling disabled")
        return

    def _build_url() -> str:
        return f"{API_BASE}/{insta_acc_id}/media?fields=id,caption,media_type,media_url,permalink,timestamp&access_token={page_token}"

    # Seed currently published media so we don't spam old posts on startup
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(_build_url()) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    for media in data.get("data", []):
                        _seen_instagram_media_ids.add(media["id"])
                    logger.info(f"[INSTAGRAM-MEDIA] Seeded {len(_seen_instagram_media_ids)} existing media posts")
                else:
                    body = await resp.text()
                    logger.warning(f"[INSTAGRAM-MEDIA] Seed request failed {resp.status}: {body}")
    except Exception as e:
        logger.warning(f"[INSTAGRAM-MEDIA] Error seeding media: {e}")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(_build_url()) as resp:
                    if resp.status != 200:
                        body = await resp.text()
                        logger.warning(f"[INSTAGRAM-MEDIA] Media fetch error {resp.status}: {body}")
                        continue
                    data = await resp.json()

            for media in reversed(data.get("data", [])):
                media_id = media.get("id")
                if not media_id or media_id in _seen_instagram_media_ids:
                    continue
                _seen_instagram_media_ids.add(media_id)

                permalink = media.get("permalink", "")
                caption = media.get("caption", "").strip()
                media_type = media.get("media_type", "")

                logger.info(f"[INSTAGRAM-MEDIA] New post detected: {permalink} ({media_type})")

                # Post link to Discord text channel
                target_channel_id = 451580655650996236
                text_channel = bot.get_channel(target_channel_id)
                if not text_channel:
                    try:
                ...
                if text_channel:
                    msg_text = f"#### 📱 *El Indio publicó en Instagram:* {permalink}"
                    if caption:
                        msg_text += f"\n> {caption}"
                    await text_channel.send(msg_text)

                # If it's a video (Reel), stream it to voice if there is anyone in a voice channel
                if media_type == "VIDEO" or "/reel/" in permalink:
                    ...
                    if candidates:
                        ...
                        if text_channel:
                            await text_channel.send(f"*Reproduciendo el nuevo Reel en {voice_channel.name}...*")

                        success, status_msg = await start_instagram_reel_stream_logic(
                            guild.id, voice_channel, permalink, sender_name="Indio"
                        )
                        if not success and text_channel:
                            if status_msg == "busy":
                                await text_channel.send("⚠️ Ya hay una transmisión de pantalla activa. Esperá a que termine.")
                            else:
                                await text_channel.send(status_msg)

        except Exception as e:
            logger.error(f"[INSTAGRAM-MEDIA] Unhandled error: {e}")
