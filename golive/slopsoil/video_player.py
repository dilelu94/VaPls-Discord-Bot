...
def _extract_subtitle_file(url: str, sub_idx: int, timeout: float = 12.0) -> str | None:
    ...
    def _send_frame(self, nal_units: list[bytes]) -> None:
        ...
        for i, payload in enumerate(all_payloads):
            ...
            try:
                ...
                elif hasattr(conn, "send_packet"):
                    conn.send_packet(packet)
                elif hasattr(conn, "socket") and conn.socket:
                    conn.socket.sendall(packet)
                self._packets_sent += 1
            except Exception as exc:
                if self._packets_sent < 5:
                    log.warning("Video packet error (seq=%d): %s", self._seq, exc)
                else:
                    log.debug("Video packet dropped (seq=%d): %s", self._seq, exc)
    ...
    def _stream(self) -> None:
        ...
        while not self._end.is_set():
            ...
            for raw in parts[:-1]:
                ...
                if nt == _NAL_AUD:
                    # AUD: explicit Access Unit (frame) boundary
                    if frame:
                        if _emit(frame):
                            _stop = True
                            break
